...
async def interactive_mode():
    ...
    shared = await process_input(natural_language, markdown_files)
    ...
    while True:
        ...
        if feedback.strip().lower() == "q":
            ...
            # 健壮性检查：为空或不是.md结尾，或全是横线等，自动加日期
            today = datetime.datetime.now().strftime("%Y%m%d")
            if (
                not filename
                or not filename.endswith(".md")
                or filename in [".md", "-", "--.md", "documentation.md"]
            ):
                filename = f"documentation-{today}.md"
            output_dir = "output"
